import { findProductById } from "@/app/actions"
...
export default async function ProductView({ id }: Props) {
  ...
  const [productFound, setProductFound] = useState('loading')

    useAsyncEffect(async () => {
      const newProduct = await findProductById(id)
      setProduct(newProduct)
      setProductFound(newProduct ? 'found' : 'notFound')
    }, [id])
  ...
  return product ?        <div className="w-full flex bg-commerce-black flex-1 h-full items-start justify-center">
  <div
